...
function checkViolations(filePath, content) {
  ...
  const hasRadiusLimit =
    /\$maxDistance|radius_km|radiusKm|maxRadiusKm|5000|distance_km/i.test(
      content,
    );

  const directLatLngComparison =
    /(lat|latitude|lng|lon|longitude)\s*(>=|<=|>|<)\s*-?\d+(\.\d+)?/i.test(
      content,
    ) ||
    /(lat|latitude|lng|lon|longitude)\s*(>=|<=|>|<)\s*[A-Za-z_][A-Za-z0-9_]*/i.test(
      content,
    );

  if (directLatLngComparison) {
    violations.push(
      "Direct lat/lng comparison detected; use geo operators on indexed fields.",
    );
  }
  ...
  if ((hasNear || hasGeoNear || hasGeoWithin) && !hasRadiusLimit) {
    violations.push(
      "Geo query appears to be missing an explicit distance/radius limit.",
    );
  }

  const hasIndexedFieldHint =
    /2dsphere|createIndex\(|location|coordinates|geo_location|geoLocation|GEO_INDEXED_FIELD/i.test(
      content,
    );

  if ((hasNear || hasGeoNear || hasGeoWithin) && !hasIndexedFieldHint) {
    violations.push(
      "Geo query detected without indexed-field hint (location/coordinates/2dsphere).",
    );
  }
